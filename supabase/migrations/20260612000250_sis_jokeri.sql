-- Paket 32 A — SİS jokeri: Klasik Mod'da Savunma Kilidi'nin yerine (sahibinin kararı, 18 Eyl 2026).
--
-- "Savunma kalkanı olayı klasik modda pek iyi durmadı": kalkan boşa gidebiliyordu (rakip
-- joker basmayacaksa işe yaramıyor) ve etkisi görünmüyordu. Sis her zaman etkili, anında görülür.
-- Önce YALNIZ Klasik Mod'da. Düellonun joker mantığı (duello_* fonksiyonları) DEĞİŞMEDİ;
-- savunma_kilidi türü, envanterdeki adetler ve düellodaki kullanımı AYNEN duruyor.
--
-- Etki deseni Paket 31'inkiyle aynı: kayıt joker_kullanimlari'nda, yeni tablo YOK.
--   * rakibin ekranı: joker_mac_durumu.sis_bitis (+ sunucu_zamani) → perde bu ana kadar iner
--   * sunucu güvencesi: sis sürerken submit_match_answer cevabı reddeder; bot da cevaplamaz
--   * son klasik_sis_son_esik_sn (6) saniyede Sis reddedilir — kural YALNIZ Sis için
--   * bot da Sis basar (aynı son-6-saniye kuralıyla)
...
insert into public.oyun_ayarlari (anahtar, deger, aciklama) values
  ('klasik_sis_sn', '3'::jsonb, 'Klasik Mod Sis: rakibin ekranında sisin kalma süresi (sn)'),
  ('klasik_sis_son_esik_sn', '6'::jsonb, 'Klasik Mod Sis: soru süresinin son bu kadar saniyesinde Sis kullanılamaz'),
  ('coin_joker_sis', '80'::jsonb, 'Sis jokeri maç içi / dükkân fiyatı (coin) — Savunma Kilidi ile aynı')
on conflict (anahtar) do nothing;

-- Fiyat tek kaynak (Paket 28): tür listesine sis
...
as $function$
  select jsonb_object_agg(t, public.joker_fiyati(t))
    from unnest(array['elli', 'sure', 'soru_degistir',
                      'zaman_baskisi', 'saldiri_degistir', 'savunma_kilidi', 'sis']) t;
$function$;

-- Rakibin ekranı için: + sis_bitis, sunucu_zamani; çubuk için kullanilan_turler (dönüş tipi değişti → drop + create)
...
returns table(sinir integer, kullanilan integer, ucretsiz_elli_kaldi boolean,
              kilitli boolean, kisaltildi boolean, sis_bitis timestamptz, sunucu_zamani timestamptz,
              kullanilan_turler text[])
language sql stable security definer set search_path to 'public'
as $function$
  with etki as (
    select k.tur, k.created_at
  ...
  select
    ...
    (select max(created_at) + make_interval(secs => public.ayar_sayi('klasik_sis_sn', 3))
       from etki where tur = 'sis'),
    now(),
    -- B.2: "kullanıldı" durumu soru değişince kaybolmasın (bileşen her soruda yeniden kurulur)
    coalesce((select array_agg(distinct k.tur) from public.joker_kullanimlari k
...
-- ---------------------------------------------------------------------------
-- Canlı gövdeler + Paket 32 ekleri (başka satır değişmedi)
...
begin
  ...
  elsif p_tur in ('savunma_kilidi', 'sis') then
    -- Sis (Paket 32): kayıt joker_kullanimlari'nda; rakibin ekranı joker_mac_durumu.sis_bitis'i,
    -- submit_match_answer sis süresince cevabı reddeder. savunma_kilidi eski maçlar için duruyor.
...
begin
  ...
  loop
    ...
    select t into v_tur
      from unnest(array['zaman_baskisi', 'sis', 'soru_degistir']) t   -- Paket 32: kilit yerine Sis
     where not exists (select 1 from public.joker_kullanimlari k
       ...
       -- Sis'in son N saniye kuralı bota da geçerli
       and (t <> 'sis' or now() <= (select m2.soru_baslangic from public.matches m2 where m2.id = r.id)
                                   + make_interval(secs => 15 - public.ayar_sayi('klasik_sis_son_esik_sn', 6)))
...
begin
  ...
  foreach v_tur in array array['elli', 'sure', 'soru_degistir',
                               'zaman_baskisi', 'saldiri_degistir', 'savunma_kilidi',
                               'seri_koruma', 'sis'] loop   -- Paket 32: Sis
...
begin
  ...
  -- Paket 32 A.2: rakibin Sisi sürerken cevap yok (istemci de kilitliyor; bu sunucu güvencesi)
  if v_senkron and exists (
    select 1 from public.joker_kullanimlari k
     where k.mac_tur = '1v1' and k.mac_id = p_match_id and k.soru_index = v_index
       and k.tur = 'sis' and k.user_id <> auth.uid()
       and k.created_at + make_interval(secs => public.ayar_sayi('klasik_sis_sn', 3)) > now()
  ) then
    raise exception 'Sis kalkınca cevaplayabilirsin';
  end if;
...
-- ---------------------------------------------------------------------------
-- A.1.6: Mevcut gerçek hesaplara tek seferlik Sis (Paket 29 D deseni).
-- Ayrım noktası: kullanıcının tur='sis', kaynak='baslangic' hareketi varsa tekrar verilmez.
-- Botlara verilmez. Dağıtım joker_hareket() üzerinden (envantere doğrudan yazılmaz).
create or replace function public.sis_baslangic_dagit()
    ...
    select p.id from public.profiles p
     where not coalesce(p.is_bot, false)
       and not exists (select 1 from public.joker_islemleri i
                        where i.user_id = p.id and i.kaynak = 'baslangic' and i.tur = 'sis')
     order by p.created_at
       for update of p
  loop
    perform public.joker_hareket(v_user, 'sis', v_adet, 'baslangic', 'paket32_sis');
    oyuncu := oyuncu + 1;
    joker := joker + v_adet;
  end loop;
  return next;
...
$function$;
revoke all on function public.sis_baslangic_dagit() from public, anon, authenticated;

select * from public.sis_baslangic_dagit();
